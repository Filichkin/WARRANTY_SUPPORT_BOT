...
def generate_chroma_db() -> Optional[Chroma]:
    ...
    try:
        ...
        if not documents:
            logger.warning(
                'Нет документов для добавления в базу данных'
                )
            return None
        ...
        logger.success(
            f'База Chroma инициализирована, добавлено {len(all_chunks)} '
            f'чанков из {len(documents)} документов'
        )
        return chroma_db
    except Exception as e:
